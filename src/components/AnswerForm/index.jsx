...
function AnswerForm({ type, questionId, feedName, feedProfile, onSubmit }) {
  ...
  useEffect(() => {
    if (type === 'edit') {
      fetch(`/api/answers_read/${questionId}`)
        ...
        .then((data) => console.log(data)) // 기존 답변을 폼에 설정
  ...
  const handleFormSubmit = (event) => {
    ...
    // fetch(url, {
    //   method: type === 'create' ? 'POST' : 'PATCH', //@crud 데이터 관리 핵심인데 그것에 관한 메서드 - 다시 한번 봐보기@
  ...
  return (
    <div className={styles.answerForm}>
      ...
      <div className={styles.header}>
        ...
        {/* (3)이미지를 보여준다 */}
        <span onClick={handleClickHello}>{feedName}</span> {/* (4)주제명 feedName을 보여준다 */}
      </div>

      {/* (5)답변폼을 만든다 */}
      <form onSubmit={handleFormSubmit}>
        {/* (6)답변 입력을 만든다 */}
        <textarea className={styles.textarea} placeholder="답변 입력" />
